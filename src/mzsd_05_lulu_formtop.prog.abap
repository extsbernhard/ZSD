*&---------------------------------------------------------------------*
*& Include MZSD_05_LULU_FORMTOP                              Modulpool        SAPMZSD_05_LULU_FORM
*&
*&---------------------------------------------------------------------*

PROGRAM  sapmzsd_05_lulu_form.
...
TYPES: BEGIN OF ty_lulu_fakt.
        INCLUDE TYPE zsd_05_lulu_fakt.
TYPES:  verrg_beginn TYPE	ld_perst,
        verrg_ende TYPE	ld_pered,
        flag    TYPE c,
        action  TYPE c,  " Modify (M) = Insert oder Update ; Delete (D) = Löschen
        delable TYPE c,  " X = löschbar ohne DB-Update
       END OF ty_lulu_fakt.
