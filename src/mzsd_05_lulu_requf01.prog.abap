...
FORM obj_period_exists  USING    uv_fallnr
  ...
  IF NOT uv_fallnr IS INITIAL.
    SELECT SINGLE * FROM zsd_05_lulu_hd02 INTO gs_lulu_head
...
FORM print_confirm TABLES tt_lulu_fakt LIKE gt_lulu_fakt
  ...
  DATA: lv_sfname TYPE tdsfname,
        ...
        ls_lulu_head TYPE zsd_05_lulu_hd02,
  ...
  ls_sf_options-tdsuffix1       = 'BRAF'.
  ls_sf_options-tdtitle         = 'LULU Brief/Angabenformular'.
...
FORM fall_enqueue USING uv_mode
  ...
  CALL FUNCTION 'ENQUEUE_EZSD_05_LULUREQU'
    EXPORTING
      mode_zsd_05_lulu_hd02 = uv_mode
      mode_zsd_05_lulu_fk02 = uv_mode
      mandt                 = sy-mandt
      fallnr                = uv_fallnr
*     VBELN                 = VBELN
      _scope                = '2'
  ...
  IF sy-subrc <> 0.
    ...
    MESSAGE s027(zsd_05_lulu) WITH uv_fallnr lv_enqusr.
  ENDIF.
