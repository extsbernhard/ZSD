...
*>>> new insert >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"Epo20141015
  if p_novfg eq c_activ.
*    d.h. nur die Fälle lesen, die noch nicht verfügt sind  ...
     select * from zsd_05_lulu_hd02
       into CORRESPONDING FIELDS OF TABLE t_worktab
      where vfgdt   eq c_initial "Verfügungsdatum ist nicht bestückt!!!
        and obj_key in s_objkey  "der Objektschlüssel Sinn !!!
        and belnr   eq space.    " und die Belegnummer muss leer sein!
  elseif p_nofib eq c_activ.
*    d.h. nur die Fälle lesen, die noch kein FI-Beleg haben ...
     select * from zsd_05_lulu_hd02
       into CORRESPONDING FIELDS OF TABLE t_worktab
      where vfgdt   in s_vfgd1   "hier macht nur Verfügungsdatum
        and obj_key in s_objkey  "der Objektschlüssel Sinn !!!
        and belnr   eq space.    " und die Belegnummer muss leer sein!
*<<< end of insert <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"Epo20141015
  elseif p_fibel eq c_activ  " aus if wurde elseif ....   "Epo20141015
      or p_ausgl eq c_activ.
...
*
*>>> new insert >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"Epo20141015
 if p_novfg eq c_activ. "wenn keine Verfügung, kein LuLu-Prof ...
...
*             and fallnr    eq t_worktfak-fallnr nicht vorhanden!!!
             and obj_key   eq t_worktfak-obj_key.
    check sy-subrc eq 0.
 else.   "wenn Verfügung erstellt, sollten die Protokoll-Daten da sein
*<<< end of insert <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"Epo20141015
    select * from ZSD_05_LULU_PROF "Prot-Datei aus Druck-Verfügungen
*    select * from zsd_05_kehr_aufz
"epo20141001
             into CORRESPONDING FIELDS OF TABLE t_worktdet
             for ALL ENTRIES IN t_worktfak
           where vbeln     eq t_worktfak-vbeln
             and fallnr    eq t_worktfak-fallnr
             and obj_key   eq t_worktfak-obj_key.
    check sy-subrc eq 0.
*>>> new insert >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"Epo20141015
 endif.
*<<< end of insert <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"Epo20141015
 ...
 loop at t_worktdet into s_worktdet.
   move s_worktdet-vgubtr to s_worktdet-vgubtr_bru.
   move s_worktdet-erdat  to s_worktdet-RUE_BASIS_DT.
* >> Folgezeilen wurden unnötig, das ...LuLu_Prof die Daten liefert <<
...
*
 check w_lines > 0.
*>>> new insert >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"Epo20141015
 check p_nofib ne c_activ. "ohne FI-Beleg darf nicht gewählt sein
 check p_novfg ne c_activ. "ohne Verfügung darf nicht gewählt sein
*<<< end of insert <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"Epo20141015
*
 loop at t_worktab into s_worktab.
...
*
 loop at t_worktab into s_worktab. "Fälle bzw. Gesuche-Kopfdaten
  clear: lw_rubtr_fi, lw_vgubtr_fi, lw_rubtr, lw_vgubtr,
         lw_kst_fi, lw_vst_fi.
*>>> new insert >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"Epo20141015
  if  not p_nofib eq c_activ                              "Epo20141015
  and not p_novfg eq c_activ.                             "Epo20141015
*<<< end of insert <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"Epo20141015
...
*    keine FI-Beleg-Daten, keine FI-Belegwert, irgendwie logisch, oder?
     clear: lw_rubtr_fi, lw_vgubtr_fi, lw_vst_fi, lw_kst_fi.
*    also leer machen ... putzen, ... cleanen bzw. clearen ... :-)
  endif.
*<<< end of insert <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<"Epo20141015
